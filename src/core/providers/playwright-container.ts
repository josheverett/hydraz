...
export async function ensurePlaywrightContainerRuntime(
  ...
  const healthCommand = [
    ...
    `mkdir -p ${shellEscape(binDir)} && ln -sfn ${shellEscape(executablePath)} ${shellEscape(exposedExecutable)} && printf healthy`,
  ...
  const provisionCommand = [
    ...
    `ln -sfn ${shellEscape(executablePath)} ${shellEscape(exposedExecutable)}`,
  ].join(' && ');
