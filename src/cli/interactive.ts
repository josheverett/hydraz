...
export async function runInteractive(): Promise<void> {
  ...
  if (allSessions.length > 0) {
    choices.push({ name: 'Inspect session', value: 'review' });
  }
...
async function newSessionFlow(repoRoot: string, repoName: string): Promise<void> {
  ...
  const goal = await input({
    message: 'Goal',
    validate: (val) => val.trim().length > 0 || 'Goal cannot be empty.',
  });
...
async function reviewFlow(repoRoot: string): Promise<void> {
  ...
  console.log(`  Branch:        ${session.branchName}`);
  console.log(`  Target:        ${session.executionTarget}`);
  if (session.codex?.threadId) console.log(`  Codex thread:  ${session.codex.threadId}`);
