...
**Status:** v2 development is in progress. Phases 1-9 are complete (types, state machine, artifacts, investigator, architect, planner, consensus loop, worker fan-out, merge fan-in, review panel, review aggregator, feedback routing, controller integration, CLI flags). Phase 10 (resume and checkpoint support) is next. The implementation plan is in `specs/hydraz_v2_plan.md`.
