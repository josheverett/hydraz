...
**Status:** v2 development is in progress. Phases 1-8 are complete (types, state machine, artifacts, investigator, architect, planner, consensus loop, worker fan-out, merge fan-in, review panel, review aggregator, feedback routing). Phase 9 (controller integration, CLI surface, delivery) is next. The implementation plan is in `specs/hydraz_v2_plan.md`.
